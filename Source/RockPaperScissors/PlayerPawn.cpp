...
void APlayerPawn::Tick(float DeltaTime)
{
	...
	if (bHit)
	{
		...
		if ((HitActor->IsA(ARPSChoice::StaticClass())) && Cast<ARPSChoice>(HitActor)->ActorHasTag(TEXT("Choice")))
		{
...
void APlayerPawn::MakeChoice(uint8 choice)
{
	...
	SetPlayerChoiceRotation(GetActorRotation());
	if ((uint8)choice != 255)
	{
		...
		for (TActorIterator<ARPSChoice> ChoiceActor(GetWorld()); ChoiceActor; ++ChoiceActor)
		{
			...
			FString ChoiceTypeAsString = (choice == 0) ? TEXT("Rock") : (choice == 1) ? TEXT("Paper")
																					  : TEXT("Scissors");
