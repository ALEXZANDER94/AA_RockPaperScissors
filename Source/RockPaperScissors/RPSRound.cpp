...
void URPSRound::OpponentTurn()
{
	if (!Opp)
	{
		...
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), AOpp::StaticClass(), OpponentActors);
		Opp = Cast<AOpp>(OpponentActors[0]);
	}
...
void URPSRound::PerformRoundAnim()
{
    ...
    Player->GetChoiceObject()->ToggleIntensity(50000.f);
    Opp->GetChoiceObject()->ToggleIntensity(50000.f);
...
void URPSRound::CleanupRoundAnim()
{
	...
	FTimerHandle RoundStatusTimer;
	FTimerDelegate RoundStatusReset;
	RoundStatusReset.BindUFunction(this, "SetPlayerWon", EStatus::Pending);
	GetWorld()->GetTimerManager().SetTimer(RoundStatusTimer, RoundStatusReset, 1.5f, false);
    ResolveRound();
}
...
void URPSRound::ResolveRound()
{
    ...
    EStatus ePlayerWon = EStatus::Pending;
    ...
    */
	...
	AppendRound(Player->GetChoice(), Opp->GetChoice(), ePlayerWon);
    Cast<ARPS>(UGameplayStatics::GetGameMode(GetWorld()))->ToggleRoundResult();
    Opp->SetChoice(EType::None);
...
void URPSRound::CheckGame()
{
    ...
    switch (Mode)
    {
		case EMode::BOf3:
			if (Round == 3)
			{
				GameResult = (PlayerWins == PlayerLosses) ? EStatus::Draw : (PlayerWins > PlayerLosses) ? EStatus::Win
																										: EStatus::Lose;
